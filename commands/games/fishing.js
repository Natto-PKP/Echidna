...
module.exports = {
	...
	exec: function ({ message, Database }) {
		...
		if (doc.content.fishing.cooldown && doc.content.fishing.cooldown - Date.now() > 0) return message.channel.send("🧶 Rien n'a encore mordu, patientez encore un peu.")

		const drop = loot('fishing', doc.content.fishing.tool)
		...
		eco.set((gold ? Math.ceil(drop.money * 1.4) : drop.money) + eco.content.money, { path: 'money' }).save()

		const item = doc.content.fishing.inventory.find(([id]) => id == drop.id)
		item ? item[gold ? 2 : 1]++ : doc.content.fishing.inventory.push([drop.id, ...(gold ? [0, 1] : [1, 0])])
