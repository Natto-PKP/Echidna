...
content = ({ menu, client, doc }) => {
	...
	return {
		author: { name: '[Background inventory] - ' + image.name + (image.path == doc.content.wallpapers.active ? ' (Déjà équipée)' : '') },
		description: '```diff\n+ ' + image.tags.join(', ') + '```\n```css\nfont: ' + image._colors.font + '\naccent: ' + image._colors.accent + '```',
		color: image.path == doc.content.wallpapers.active ? client.colors.active : client.colors.shop,
...
module.exports = {
	...
	exec: async function ({ client, message, Database, args }) {
		...
		const _wallpapers = wallpapers.filter(({ path }) => doc.content.wallpapers.arr.includes(path))
		...
		if (images.length > 1) {
			...
			menu.collector.on('collect', async (reaction, user) => {
				...
				if (reaction.emoji.name === '🏷') menu.images[menu.page].path != doc.content.wallpapers.active && doc.set(menu.images[menu.page].path, { path: 'wallpapers.active' }).save()
			...
			const image = menu.images[menu.page]
			if (doc.content.wallpapers.active == image.path) return
