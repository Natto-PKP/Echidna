...
module.exports = {
	...
	exec: async function ({ message, args, Database }) {
		...
		const wallpaper = wallpapers.find(({ path }) => docs.profile.content.wallpapers.active == path)
		...
		ctx.fillStyle = 'rgb(10, 10, 10)'
		ctx.fill()
		ctx.roundRect(288, 333, docs.exp.content.exp / exp(docs.exp.content.level) * 100 * 3.52 > 20 ? docs.exp.content.exp / exp(docs.exp.content.level) * 100 * 3.52 : 20, 60, 8)
		...
		const levelContentWidth = ctx.measureText(docs.exp.content.level).width
		...
		ctx.stroke()
		ctx.fillStyle = wallpaper._colors.accent
		ctx.fillText(docs.exp.content.level, 282 + (45 + levelContentWidth) / 2 - levelContentWidth / 2, 314)
		...
		ctx.fillText(docs.eco.content.tickets.form(), x + 47, 306)
		ctx.strokeText(docs.eco.content.tickets.form(), x + 47, 306)

		// Money
		ctx.drawImage(await loadImage('./_storage/icons/money.png'), (x = x + 67 + ctx.measureText(docs.eco.content.tickets.form()).width) && x, 275, 42, 42)
		ctx.fillText(docs.eco.content.money.form(), x + 52, 306)
		ctx.strokeText(docs.eco.content.money.form(), x + 52, 306)
